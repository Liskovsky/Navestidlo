...
object ColorParser {
    ...
    private val colorMap = mapOf(
        ...
        "fastOrange" to AllBlinkingColors.Blinking(Color(0xFFFFA500), Color.DarkGray, BLINK_DURATION_FAST),
        "slowOrange" to AllBlinkingColors.Blinking(Color(0xFFFFA500), Color.DarkGray, BLINK_DURATION_SLOW),
    ...
    fun parseBlinkColor(colorString: String): AllBlinkingColors {
        ...
        try {
            if (colorString.contains(",")) {
                ...
                val parts = colorString.split(",").takeIf { it.size >= 2 } ?: return AllBlinkingColors.SolidColor(Color.Gray)
                AllBlinkingColors.Blinking(
                    color1 = Color(android.graphics.Color.parseColor(parts[0])),
                    color2 = Color(android.graphics.Color.parseColor(parts[1])),
                    speed = parts.getOrNull(2)?.toIntOrNull() ?: 500
                )
            } else {

                // Pokud řetězec neobsahuje žádnou čárku, považuje se za jednoduchou barvu
                AllBlinkingColors.SolidColor(Color(android.graphics.Color.parseColor(colorString)))
            }
        } catch (e: Exception) {

            // Pokud dojde k chybě při parsování, vrátí šedou barvu
            AllBlinkingColors.SolidColor(Color.Gray)
        }
    }
}
